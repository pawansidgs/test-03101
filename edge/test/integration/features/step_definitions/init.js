...
var environment = config['test-03101'].env;
...
console.log('&test-03101 api: [' +  domains[environment] + ', ' + config['test-03101'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test-03101'].app, config['test-03101'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test-03101'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
